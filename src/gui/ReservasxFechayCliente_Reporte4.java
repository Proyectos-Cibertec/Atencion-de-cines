...
public class ReservasxFechayCliente_Reporte4 extends JDialog implements ActionListener, PropertyChangeListener, ItemListener {
	 ...
	 * Create the dialog.
	 */
	public ReservasxFechayCliente_Reporte4() {
		setModal(true);
		setTitle("Reporte - Reservas por fecha y por cliente");
		...
		cboCliente.setBounds(186, 70, 182, 20);
		panelFecha.add(cboCliente);
		
		lblCodigoCliente = new JLabel("C\u00F3digo de cliente");
		lblCodigoCliente.setBounds(26, 73, 145, 14);
		panelFecha.add(lblCodigoCliente);
		...
		scrollPane.setViewportView(txtS);
		{
			...
			getContentPane().add(buttonPane, BorderLayout.SOUTH);
			{
				...
				btnSalir.setIcon(new ImageIcon(ReservasxFechayCliente_Reporte4.class.getResource("/img/btnSalir2.png")));
	...
	// Llena los combobox con los datos de los clientes
	void llenarClientes() {
		for (int i = 0; i < ae.tamaño(); i++) {
			cboCliente.addItem(ac.obtener(i).getCodigo() + "");
		}
	}
	...
	void metodo() {
		...
		imprimir("CLIENTE: " + c.getCodigo() + " " + c.getApellidoMaterno() + " " + c.getApellidoPaterno() + ", " + 
				c.getNombres());
		...
		if (fechaInicio.length() > 0 && fechaFin.length() > 0) {
			...
			for (int i = 0; i < ar.tamaño(); i++) {
				...
				if (LibreriaFechas.estaEntreFechas(reserva.getFechaReserva(), fechaInicio, fechaFin)) {
					...
					if (cliente.getCodigo() == leerCodigoCliente()) {
						
						
						// Se contabiliza la reserva entre las fechas dadas para el cliente elegido
						contador++;
			...
			if (contador == 0) {
				imprimir("NO SE REGISTRAN RESERVAS ENTRE LAS FECHAS DADAS PARA EL CLIENTE");
			} else {
	...
	protected void itemStateChangedCboEmpleado(ItemEvent e) {
		...
		Cliente cliente = ac.buscar(codigoCliente);
		txtNombres.setText(cliente.getApellidoPaterno() + " " + cliente.getApellidoMaterno() + ", " + cliente.getNombres());
