...
public class DlgPagoReservas extends JInternalFrame implements ActionListener, MouseListener, KeyListener, ItemListener {
	...
	void modificarReserva(Reserva nuevaReserva) {
		...
		if (reserva != null) {
			...
			listar();
			habilitar(false);
			mensaje("Se pagó la reserva de manera exitosa");
			...
			mensaje("Código de la reserva a pagar no existe");
		}
	}
	
	void liberarButacasDeReserva(int codigoReserva) {
		for (int i = 0; i < adr.tamaño(); i++) {
	...
	protected void actionPerformedBtnIngresar(ActionEvent e) {		
		...
		// Pagar
			...
			//mensaje("Validar que solo sepuede pagar la reserva hasta hasta 30 minutos antes del inicio de la función");
			// VALIDAR
	...
	public void mouseClicked(MouseEvent e) {
		// Cuando se hace clic en la tabla, se activa la opción para modificar
		this.accion = PAGAR;
		...
		// Se deshabilitan los botones que permiten cambiar de Cliente, elegir algunas otros botones según sea el caso
		btnSeleccionarCliente.setEnabled(false);
